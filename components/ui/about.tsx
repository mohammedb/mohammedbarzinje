...
function FactCard({ fact, index }: { fact: Fact; index: number }) {
    ...
    return (
        <motion.div
            ...
            initial="hidden"
            animate={controls}
            whileHover={{ y: -4, transition: { duration: 0.4, ease: [0.32, 0.72, 0, 1] as const } }}
            className={`group relative rounded-[1.5rem] border border-[var(--rule-strong)] p-5 md:p-6 ${
                isFirst
        ...
        >
            <div
                className={`text-[10px] uppercase tracking-[0.22em] mb-3 ${
                    isFirst ? "text-white/55" : "text-[var(--ink-mute)]"
                }`}
            >
                {fact.label}
            </div>
            <div
                className={`text-[28px] md:text-[34px] font-medium tracking-[-0.03em] leading-[1.05] ${
                    isFirst ? "text-[var(--canvas-soft)]" : "text-[var(--ink)]"
                }`}
            >
                {fact.value}
            </div>
            <div
                className={`mt-3 text-[12px] tracking-tight ${
                    isFirst ? "text-white/60" : "text-[var(--ink-mute)]"
                }`}
            >
                {fact.note}
            </div>
